...
async function testNodeHelperPostsStructuredPayload() {
  ...
  assert.equal(httpRequest.url, "<DEBUG_URL>/log");
  assert.equal(httpRequest.options.method, "POST");
  assert.equal(httpRequest.options.headers["content-type"], "application/json");
  assert.deepEqual(JSON.parse(httpRequest.body), {
    probe: "node.post",
    hypothesis: "H1",
    session: "<session_id>",
    source: "node",
    ...
    process: {
      getBuiltinModule() {
        return {
          request() {
            requestCalls += 1;
            throw new Error("request should not run for invalid events");
          },
        };
      },
    },
